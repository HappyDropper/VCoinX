...
vCoinWS.onReceiveDataEvent(async (place, score) => {
    ...
    if (place > 0 && !rl.isQst) {
        ...
        if (transferTo && (transferCoins * 1e3 < score || transferCoins * 1e3 >= 9e9) && ((Math.floor(Date.now() / 1000) - transferLastTime) > transferInterval)) {
            try {
                ...
                if (transferCoins * 1e3 >= 9e9) {
                    ...
                    template = "Automatical transfered [" + formatScore(score * 1e3, true) + "] coins from (@id" + USER_ID + ") to @id" + transferTo;
                } else {
                    let minCoins = Math.min(score / 1000, transferCoins);
                    await vCoinWS.transferToUser(transferTo, minCoins);
                    template = "Automatical transfered [" + formatScore(minCoins * 1e3, true) + "] coins from (@id" + USER_ID + ") to @id" + transferTo;
                }
        ...
        if (!hidejunk)
            con("TOP: " + place + " Coins: " + formatScore(score, true) + " Speed: " + formatScore(vCoinWS.tick, true) + " coin/t.", "yellow");
    }
});
vCoinWS.onTransfer(async (id, score) => {
    let template = "User (@id" + USER_ID + ") get [" + formatScore(score, true) + "] coins from @id" + id;
...
vCoinWS.onUserLoaded((place, score, items, top, firstTime, tick) => {
    ...
    con("Скорость: " + formatScore(tick, true) + " coin/t.");
    if (!advertDisp) {
        ccon("VCoinX Special for Dropper Project vk.com/dropperproject by HappyDropper @happy_dropper", "black", "Green");
